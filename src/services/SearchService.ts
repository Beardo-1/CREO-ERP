...
export async function searchAll(query: string): Promise<SearchResult[]> {
  ...
  try {
    ...
    // Development: fetch all and run simple search
    ...
    // Get agents synchronously with error handling
    let agents: any[] = [];
    try {
    ...
    const items = [
      ...props.map(p => ({ 
        id: p.id, 
        label: p.title || 'Untitled Property', 
        type: 'Property', 
        route: `/properties/${p.id}` 
      })),
      ...contacts.map(c => ({ 
        id: c.id, 
        label: `${c.firstName || ''} ${c.lastName || ''}`.trim() || 'Unnamed Contact', 
        type: 'Contact', 
        route: `/contacts/${c.id}` 
      })),
      ...agents.map(a => ({ 
        id: a.id, 
        label: `${a.firstName || ''} ${a.lastName || ''}`.trim() || 'Unnamed Agent', 
        type: 'Agent', 
        route: `/agents/${a.id}` 
      })),
      ...deals.map(d => ({ 
